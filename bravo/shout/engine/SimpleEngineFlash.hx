...
class SimpleEngineFlash extends EngineBase, implements IEngine
{
	...
	var _owner : EventDispatcher;
	
	public function new(owner : EventDispatcher, host : String, port : UInt, resource : String, ?uvox : Bool = false, wantVideo : Bool = true) 
	{
		super();
			...
			case EngHeader, EngAudioData, EngVideoData, EngAudioPreData, EngVideoPreData:
				...
				if (e.msgtyp == EngHeader)
				{
					...
					if (netstream != null)
					{
						...
						{
							// dispatch an event containing netstream so the parent can _video.attachNetStream(netstream);
							_owner.dispatchEvent(new PlayerEvent(PlayerEvent.NETSTREAM_VIDEO, { 
								netstream : netstream,
							} ));
						}
					}
				...
				{
					_owner.dispatchEvent(new PlayerEvent(PlayerEvent.BUFFER_STATUS, { 
			...
			case EngInformation:
				if (e.object != null)
				{
					...
					else if (e.object.width != null && e.object.height != null && e.object.rate != null && e.object.flip != null)
					{
						if (_wantVideo)
						{
							_owner.dispatchEvent(new PlayerEvent(PlayerEvent.VIDEO_PARAMETERS, e.object));
/*							
...
*/
			...
			case EngMetaData:
				if (e.object != null)
				{
					...
					{
						_owner.dispatchEvent(new PlayerEvent(PlayerEvent.SONG_TITLE, e.object));
						dbgtrace(e.object.StreamTitle);
					}
					else if (e.object.metadata != null)
					{
						_owner.dispatchEvent(new PlayerEvent(PlayerEvent.CODEC_METADATA, e.object));
						dbgtrace(e.object.metadata);
					}
					else if (e.object.xmldata != null)
					{
						_owner.dispatchEvent(new PlayerEvent(PlayerEvent.XML_METADATA, e.object));
						dbgtrace(e.object.metadata);
					}
				}
			case EngStreamName:
				...
				{
					_owner.dispatchEvent(new PlayerEvent(PlayerEvent.STREAM_TITLE, e.object));
				}
			case EngStreamStartError:
				_owner.dispatchEvent(new PlayerEvent(PlayerEvent.STREAM_FAIL, e.object));
	...
	public function go()
	{
		...
		var uvox : Bool = false;
		lastBytesLoaded = 0;
		_owner.dispatchEvent(new PlayerEvent(PlayerEvent.SOCKET_OPEN, { } ));
	...
	{
		_owner.dispatchEvent(new PlayerEvent(PlayerEvent.SOCKET_FAIL, { } ));
		_owner.dispatchEvent(new PlayerEvent(PlayerEvent.URL_OPEN, { } ));
	...
	{
		_owner.dispatchEvent(new PlayerEvent(PlayerEvent.URL_FAIL, { } ));
